// 令和4年第5回定例会のインフォグラフィックデータをSupabaseにアップロード
...
async function uploadInfographic() {
  console.log('📤 令和4年第5回定例会のインフォグラフィックデータをアップロード中...\n');

  // JSONファイルを読み込む
  const data = JSON.parse(
    fs.readFileSync('r4-5th-infographic-data.json', 'utf-8')
  );
  ...
  if (existing) {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        supplementary_budget: data.supplementaryBudget,
        total_budget_after: data.totalBudgetAfter,
        published: true,
        display_order: 5, // 令和4年第5回
      })
      .eq('id', existing.id);
  ...
  console.log('\n🎉 アップロード完了！');
  console.log('   http://localhost:3000/meetings/令和４年第５回定例会/topics でご確認ください');
}
