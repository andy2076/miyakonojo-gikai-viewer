...
async function importTopic() {
  try {
    // JSONファイルの読み込み
    const jsonPath = path.join(process.cwd(), 'data', 'r5-2nd-session-topic-v2.json');
    const jsonData = JSON.parse(fs.readFileSync(jsonPath, 'utf-8'));

    console.log('Importing topic data (v2 - Infographic)...');
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        supplementary_budget: jsonData.budget_allocation,
        total_budget_after: jsonData.contract_allocation?.total || null,
