// 令和7年第2回定例会のインフォグラフィックデータをSupabaseにアップロード
...
async function uploadInfographic() {
  console.log('📤 令和7年第2回定例会のインフォグラフィックデータをアップロード中...\n');

  // JSONファイルを読み込む
  const data = JSON.parse(
    fs.readFileSync('r7-2nd-infographic-data.json', 'utf-8')
  );
  ...
  console.log(`  - 議案数: ${data.content_data.stats.total_bills}\n`);
  ...
  console.log('\n🎉 アップロード完了！');
  console.log('   http://localhost:3001/meetings/令和7年第2回定例会/topics でご確認ください');
}
