/**
 * 新フォーマットCSVをquestion_cardsテーブルにインポートするスクリプト
 *
 * CSVフォーマット:
 *   議員名（会派）, 大項目, 質問の要旨, 主な回答/答弁, 質問タグ
 *
 * 使い方:
 *   node scripts/import-cards-from-new-csv.js <CSVファイルパス> [--dry-run]
 */
...
function extractMeetingTitleFromPath(csvPath) {
  ...
  const match = filename.match(/令和(\d+)年第(\d+)回/);
  if (match) {
...
async function importCSV(csvPath, dryRun = false) {
  ...
  const meetingTitle = extractMeetingTitleFromPath(csvPath);
  ...
  for (const row of parsed.data) {
    const memberField = row['議員名（会派）'];
    if (!memberField) continue;

    const { memberName, faction } = parseMemberAndFaction(memberField);
    const majorItem = row['大項目'] || '';
    const questionSummary = row['質問の要旨'] || '';
    const answerRaw = row['主な回答/答弁'] || '';
    const tagsRaw = row['質問タグ'] || '';

    // タグを配列に変換
    const tags = tagsRaw.replace(/^"|"$/g, '').split(/[,、]/).map(t => t.trim()).filter(t => t);

    // テーマを作成
    const theme = {
      theme_title: majorItem,
    ...
    if (!memberData.has(memberName)) {
      memberData.set(memberName, {
        memberName,
        faction,
        themes: []
      });
    }
    memberData.get(memberName).themes.push(theme);
  }
...
if (!csvPath) {
  console.log('使い方: node scripts/import-cards-from-new-csv.js <CSVファイルパス> [--dry-run]');
  process.exit(1);
}
